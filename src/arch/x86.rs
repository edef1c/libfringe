...
pub unsafe fn init(sp: &mut StackPointer, f: unsafe extern "C" fn(usize, StackPointer) -> !) {
    ...
    sp.push(sp, trampoline_1 as usize + 2); // Return after the 2 nops
    sp.push(sp, 0xdead0cfa); // CFA slot

    // Call frame for swap::trampoline. We set up the %ebp value to point to the
    // parent call frame.
    let frame = *sp;
    sp.push(sp, trampoline_2 as usize + 1); // Entry point, skip initial nop
    sp.push(sp, frame.0 as usize); // Pointer to parent call frame
}
