...
pub unsafe trait RebuildWithTl<'a>
/*: RebuildRaw<b
    'a,
...
where
    Args: RebuildWithTl<'a>,
{
    type PayloadRaw = (Option<&'a mut ThreadLocals<Args::OldStack>>, Args::Payload);

    unsafe fn rebuild_raw(sp: StackPointer, raw_payload: Self::PayloadRaw) -> Args {
        let (maybe_stack, payload) = raw_payload;
        Args::rebuild_with_tl(sp, maybe_stack, payload)
    }
}

fn transmute_lifetime<'a, S>(
...
where
    Context<'a, Self::OldArgs, Self::OldStack>: Send,
    Self: 'a + Send,
{
    type OldArgs: RebuildRaw<'a> + 'a + Send;

    /// The function which actually does the rebuilding.
    fn rebuild(Context<'a, Self::OldArgs, Self::OldStack>, Self::Payload) -> Self;
}
...
unsafe impl<'a, Args: Rebuild<'a>> RebuildWithTl<'a> for Args {
    ...
    unsafe fn rebuild_with_tl(
        ...
        let ctx = Context {
            stack_pointer: sp,
            thread_locals: maybe_stack.map(|tl| Unique::new(tl as *mut _ as *mut _).unwrap()),
            _ref: PhantomData,
        };
...
where
    A0: Rebuild<'a, OldStack = S> + Send,
    A1: Rebuild<'a, OldStack = S> + Send,
    S: Stack + 'a + Send,
{
    type Payload = Either<A0::Payload, A1::Payload>;
    type OldStack = S;
    ...
    unsafe fn rebuild_with_tl(
        ...
        let thread_locals = maybe_stack.map(|tl| Unique::new(tl as *mut _ as *mut _).unwrap());
...
where
    TheirArgs0: Rebuild<'a, OldStack = OurStack>,
    ...
    pub fn switch_left(
        ...
        unsafe {
            self.raw_switch::<TheirArgs0::OldArgs, OurStack>((
                transmute_lifetime(maybe_stack),
                Either::Left(their_payload),
            ))
        }
    }
