...
pub type Unpacked<'a, C> = Context<'a, ArgsRaw<'a, C>, <C as Cycle<'a>>::Stack>;
pub type ArgsRawest<Stack, Next, Arg> = Either<ExitedArgs<Stack, Arg>, YieldedArgs<Next, Arg>>;
pub type ArgsRaw<'a, C> = ArgsRawest<
    <C as Cycle<'a>>::Stack,
    <C as Cycle<'a>>::Next,
    <C as Cycle<'a>>::Arg,
>;
pub type Args<'a, C> = (Option<<C as Cycle<'a>>::Next>, <C as Cycle<'a>>::Arg);
...
pub trait Cycle<'a>: Send + Sized + 'a {
    ...
    where
        F: FnOnce(&mut ThreadLocals<Self::Stack>, Args<'a, Self>) -> ! + Send + 'a,
    {
        Self::pack(Context::new(stack, |tl, a| {
            let x: ! = f(tl, pack_args::<Self>(a));
            panic!()
        }))
    }
    ...
    #[inline(always)]
    fn kontinue(self, maybe_stack: Option<&mut ThreadLocals<Self::Stack>>, args: Self::Arg) -> ! {
    ...
    Arg: 'a + Send,
{
    ...
    fn rebuild(ctx: Context<'a, Self::OldArgs, Self::OldStack>, arg: Self::Payload) -> Self {
        // println!("Switched from dead thread");
        drop(ctx);
        // println!("Dropped dead thread");
