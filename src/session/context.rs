    ...
    S: Stack + 'a,
{
    ...
    where
        F: FnOnce(&mut ThreadLocals<S>, Args) -> ! + Send + 'a,
    {
        let tl = ThreadLocals {
        ...
        let (sp, tlp): (StackPointer, Unique<ThreadLocals<S>>) = unsafe {
            ...
            fat_args::init1(rets, None, move |initializer_sp| -> ! {
                ...
                let (sp, payload) = fat_args::swap(Unique::new(&mut tl_move), initializer_sp, None);
...
where
    Context<'a, TheirArgs, TheirStack>: Send + 'a,
    ...
    TheirStack: Stack + 'a,
{
    ...
    pub(super) unsafe fn raw_switch<'b, OurArgs, OurStack>(
        self,
        their_payload: TheirArgs::PayloadRaw,
    ) -> OurArgs
    where
        'a: 'b,
        ...
        OurStack: Stack + 'b,
    {
        // println!("new stack_pointer: {:?}", self.stack_pointer);
        let (sp, our_payload) = fat_args::swap(their_payload, self.stack_pointer, None);
        // println!("old stack_pointer: {:?}", sp);
...
pub unsafe trait RebuildRaw<'a> {
    ...
    unsafe fn rebuild_raw(StackPointer, Self::PayloadRaw) -> Self
    where
        Self: 'a;
}
...
unsafe impl<'a> RebuildRaw<'a> for ! {
    type PayloadRaw = !;
    unsafe fn rebuild_raw(_sp: StackPointer, payload: !) -> ! {
        payload;
        panic!()
    }
}
    ...
    fn drop(&mut self) {
        if let Some(ptr) = ::core::mem::replace(&mut self.thread_locals, None) {
            unsafe {
                ...
                ::core::intrinsics::drop_in_place(ptr.as_ptr());
            }
        }
    }
}
